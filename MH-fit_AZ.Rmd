...
burn_in_chains = chains_dat[2001:dim(chains_dat[1]),]
```
...
beta_avg_l95 = burn_in_chains %>% summarise_at(c("V2", "V3", "V4", "V5", "V6", "V7", "V8"), 
                                               quantile, 0.0275, na.rm = T)
beta_avg_u95 = burn_in_chains %>% summarise_at(c("V2", "V3", "V4", "V5", "V6", "V7", "V8"), 
                                               quantile, 0.975, na.rm = T)

rho_avg_l95 = burn_in_chains %>% summarise_at("V9", quantile, 0.0275, na.rm = T)
rho_avg_u95 = burn_in_chains %>% summarise_at("V9", quantile, 0.975, na.rm = T)

sig_avg_l95 = burn_in_chains %>% summarise_at("V10", quantile, 0.0275, na.rm = T)
...
preds <- cbind(id = test.hurricanes, test_X, true = test_Y, preds = test.fit)
write.csv(test, "./preds2.csv")
```
